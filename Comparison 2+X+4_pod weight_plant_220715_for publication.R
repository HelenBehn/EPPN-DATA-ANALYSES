...
anova <- aov(pod_weight_plant ~ Genotype, data = comp_2X4ord)
summary(anova)
...
Tk <- group_by(comp_2X4ord, Genotype) %>%
  summarise(mean = round(mean(pod_weight_plant), digits = 1), quant = round(quantile(pod_weight_plant, probs = 0.75), digits = 1)) %>%
...
#boxplot
ggplot(comp_2X4ord, aes(Genotype, pod_weight_plant, fill = Generation, color = Generation)) + 
  geom_boxplot() +
