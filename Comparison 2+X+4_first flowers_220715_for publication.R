...
anova <- aov(first_flowers_DAS ~ Genotype, data = comp_2X4ord)
summary(anova)
...
Tk <- group_by(comp_2X4ord, Genotype) %>%
  summarise(mean = round(mean(first_flowers_DAS), digits = 1), quant = round(quantile(first_flowers_DAS, probs = 0.75), digits = 1)) %>%
...
#boxplot
ggplot(comp_2X4ord, aes(Genotype, first_flowers_DAS, fill = Generation, color = Generation)) + 
  geom_boxplot() +
  labs(x = "Genotype", y = "First flowers (DAS)") +
  geom_text(data = ANOVA_Tukey_2X4, aes(x = Genotype, y = quant, label = cld), color = "black", size = 2.5, vjust = 0, hjust = 7) +
  coord_flip() +
