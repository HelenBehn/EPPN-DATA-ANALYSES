...
anova <- aov(TGW_g ~ Genotype, data = comp_2X4ord)
summary(anova)
...
Tk <- group_by(comp_2X4ord, Genotype) %>%
  summarise(mean = round(mean(TGW_g), digits = 1), quant = round(quantile(TGW_g, probs = 0.75), digits = 1)) %>%
...
#boxplot
ggplot(comp_2X4ord, aes(Genotype, TGW_g, fill = Generation, color = Generation)) + 
  geom_boxplot() +
