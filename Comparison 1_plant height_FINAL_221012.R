...
ggplot(comp_1X, aes(Genotype, plant_height, fill = Generation, color = Generation)) + 
  ...
  geom_text(data = ANOVA_Tukey_1X, aes(x = Genotype, y = quant, label = cld), color = "black", size = 3.5, vjust = 0.3, hjust = 3) +
  coord_flip() +
...
#pick spec colour
#boxplot ## plant_height ###
ggplot(comp_1X, aes(Genotype, plant_height, fill = Generation, color = Generation, alpha = 0.5, guide = "none")) + 
  geom_boxplot() +
  labs(x="Genotype", y="Plant height (cm)") +
  ...
  theme(axis.title = element_text(size = 14),
        ...
        axis.text.x = element_text(size = 6),
        axis.text.y = element_text(size = 6)) +
  scale_color_manual(values = c("#440154FF", "#1F968BFF", "#FDE725FF", "#287D8EFF"), 
                                breaks = c("B. rapa (P1)", "B. oleracea (P2)", "Resynth. B. napus", "Natural B. napus"),
                                labels = c("B. rapa (P1)", "B. oleracea (P2)", "Resynth. B. napus", "Natural B. napus")) +
  scale_fill_manual(values = c("#440154FF", "#1F968BFF", "#FDE725FF", "#287D8EFF"),
                    breaks = c("B. rapa (P1)", "B. oleracea (P2)", "Resynth. B. napus", "Natural B. napus"),
                    labels = c("B. rapa (P1)", "B. oleracea (P2)", "Resynth. B. napus", "Natural B. napus"))
  
####SO GEHT'S!!! NUR ALPHA-LEGENDE LÖSCHEN   ###
ggplot(comp_1X, aes(Genotype, plant_height, fill = Generation, color = Generation, alpha = 0.5)) + 
  geom_boxplot() +
  labs(x="Genotype", y="Plant height (cm)") +
  geom_text(data = ANOVA_Tukey_1X, aes(x = Genotype, y = quant, label = cld), color = "black", size = 3.5, vjust = 0.3, hjust = 3, alpha = 1) +
  coord_flip() +
