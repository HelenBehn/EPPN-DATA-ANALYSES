...
anova <- aov(pod_weight_plant ~ Genotype, data = comp_2XY4)
summary(anova)
...
Tk <- group_by(comp_2XY4, Genotype) %>%
  summarise(mean = round(mean(pod_weight_plant, na.rm = TRUE), digits = 1), quant = round(quantile(pod_weight_plant, probs = 0.75, na.rm = TRUE), digits = 1)) %>%
...
#boxplot pod_weight_plant 2XY4
ggplot(comp_2XY4ord, aes(Genotype, pod_weight_plant, fill = Line, color = Line)) + 
  geom_boxplot() +
  labs(x="Genotype", y="Pod weight/plant (g)") +
